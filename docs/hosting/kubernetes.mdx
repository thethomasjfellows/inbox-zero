...
To use Codex CLI as the LLM provider, build a Codex-enabled image and mount
Codex auth/config from a Kubernetes Secret. This path does not use
`OPENAI_API_KEY`.

Build and push an image with a pinned Codex CLI version:
...
docker push <registry>/inbox-zero:<tag>-codex
```

Create the Codex auth Secret after running `codex login` locally. Verify the
files used by your installed Codex CLI before creating the Secret.
...
Use the example overlay as a starting point:

```bash
cp charts/inbox-zero/values-codex-cli.example.yaml values.codex.yaml
```

Set the image, public URL, database, Redis, OAuth, and Pub/Sub values for your
cluster, then install with both your production values and the Codex overlay:
...
Before installing, inspect the render:

```bash
helm template inbox-zero ./charts/inbox-zero \
  -n inbox-zero \
  -f values.prod.yaml \
  -f values.codex.yaml
```

The rendered web and worker Deployments should include `CLI_LLM_ENABLED=true`,
`CODEX_CLI_PATH=/usr/local/bin/codex`, and Codex config mounts. The migration
Job should not mount the Codex Secret.
