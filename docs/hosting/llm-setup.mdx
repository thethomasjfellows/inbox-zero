...
pnpm add -g @openai/codex@0.142.4
codex login
```
...
For Kubernetes, use the Codex-enabled image and Secret-mounted auth/config
instead of installing packages inside a running pod. This path uses Codex CLI
authentication, not `OPENAI_API_KEY`.

Build an image with the Codex CLI binary pinned:
...
After running `codex login` locally, create a Kubernetes Secret from the Codex
auth/config files your installed CLI uses. Verify the paths on your machine
...
If your Codex CLI installation uses an additional config file, include it too:
...
Then enable the chart's Codex block:
...
Use this only for trusted self-hosted deployments. Do not commit Codex auth
files, bake them into the image, or expose this as a multi-tenant public CLI
execution service.
