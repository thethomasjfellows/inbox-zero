...
| `CODEX_CLI_ALLOW_NPX` | No | Allows the Codex community provider to fall back to `npx @openai/codex` if `codex` is not on PATH. Leave disabled for Kubernetes production. | `false` |
| `CODEX_CLI_PATH` | No | Path to the `codex` binary when using `codex-cli`. The Codex-enabled production image installs it at `/usr/local/bin/codex`. | — |
| `codexCli.existingSecret` | No | Helm value for mounting Codex CLI auth/config files from a Kubernetes Secret into web and worker pods. Use this instead of storing Codex auth in env vars. | — |
| **AI Content Controls** ||||
